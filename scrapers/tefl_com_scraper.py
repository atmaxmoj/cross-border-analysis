#!/usr/bin/env python3
"""
TEFL.com Job Board Scraper
Collects China job postings from TEFL.com
"""
...
def scrape_tefl_com(page, max_pages=20):
    """Scrape TEFL.com China job board"""
    print(f"\n📊 Scraping TEFL.com China Jobs...")

    jobs = []
    # TEFL.com uses country parameter for China jobs
    base_url = 'https://www.tefl.com/job-seeker/index.html?country=44'

    for page_num in range(1, max_pages + 1):
        # TEFL.com pagination pattern
        if page_num == 1:
            page_url = base_url
        else:
            # Common patterns: ?page=N or /page/N
        ...
        try:
            ...
            for selector in job_selectors:
                ...
                if elements and len(elements) > 0:
                    # Filter valid job postings
                    valid_elements = [e for e in elements if e.query_selector('h2, h3, .title')]
                    if valid_elements:
            ...
            if not job_elements:
                ...
                # Save debug HTML
                html = page.content()
                with open(OUTPUT_DIR / f'debug_tefl_page_{page_num}.html', 'w') as f:
                    f.write(html)
                break
            ...
            for elem in job_elements:
                try:
                    # Title
                    title_elem = elem.query_selector('h2, h3, .title, .job-title')
                    title = title_elem.inner_text().strip() if title_elem else 'N/A'

                    # School/Company
                    school_selectors = ['.company', '.school', '.employer', '[class*="company"]']
                    school = 'N/A'
                    ...
                    # Date
                    date_selectors = ['.date', '.posted', 'time', '[class*="date"]']
                    date_str = 'N/A'
...
def main():
    print("🔍 Starting TEFL.com Scraper...")
    ...
    with sync_playwright() as p:
        ...
        jobs = scrape_tefl_com(page, max_pages=20)

        browser.close()

    # Save raw data
    jobs_file = OUTPUT_DIR / 'tefl_com_jobs.json'
